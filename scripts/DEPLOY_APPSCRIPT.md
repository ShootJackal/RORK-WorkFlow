...
7. Under **Version**, choose **New version** (optional: add description e.g. "v4.2 sheet name alignment").
...
| `getLeaderboard` | `period` (optional: `thisWeek`, `lastWeek`) | `{ rank, collectorName, hoursLogged, tasksCompleted, tasksAssigned, completionRate, region }[]` | Collector Task Assignments Log (+ CA_TAGGED for region), filtered by week when period set |
| `getCollectorStats` | `collector` | Collector stats + weekly hours, top tasks | Collector Task Assignments Log, CA_TAGGED, Collectors |
| `getTodayLog` | `collector` | Today’s assignments + active | Collector Task Assignments Log |
| `getRecollections` | — | Task names needing recollection | Task Actuals \| Redashpull (or Collector Actuals \| RedashPull) |
| `getFullLog` | `collector` (optional) | All assignments (optionally filtered) | Collector Task Assignments Log |
| `getTaskActualsSheet` | — | Task actuals rows (collected/good/remaining hrs, status) | Task Actuals \| Redashpull (or Collector Actuals \| RedashPull) |
| `getAdminDashboardData` | — | Tasks/recollect counts, collector summary, `activeRigsToday` | Task Actuals \| Redashpull (or Collector Actuals \| RedashPull), Collectors, RS_Task_Req, CA_TAGGED |
...
- **Collector stats**: Weekly hours from **Collector Task Assignments Log** (week start = Monday); total can be topped up from **CA_TAGGED** and **Collectors** for all-time only.
